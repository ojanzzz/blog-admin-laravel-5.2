...
!function ($) {
    ...
    BootstrapTable.prototype.resetView = function (params) {
        ...
        if (this.options.height) {
            ...
            this.$container.find('.fixed-table-container card-1').css('height', height + 'px');
        }

        if (this.options.cardView) {
            // remove the element css
            that.$el.css('margin-top', '0');
            that.$container.find('.fixed-table-container card-1').css('padding-bottom', '0');
            return;
        }
        ...
        if (this.options.height && this.options.showHeader) {
            this.$container.find('.fixed-table-container card-1').css('padding-bottom', '37px');
        }
    };
...
function Posts(value, row, index) {
  var csrf_token = $('meta[name="_token"]').attr('content');
  return "<div class='flex-group'><a class='item-flex-group' href='/admin/mbojo-edit/"+row.id+"'>Edit <svg class='small-icon glyph stroked pencil'><use xlink:href='#stroked-pencil'></use></svg></a><button class='item-flex-group btn-hidden' type='button' data-toggle='modal' data-target='#"+row.id+"'>Delete <svg class='small-icon glyph stroked trash'><use xlink:href='#stroked-trash'></use></svg></button><div id='"+row.id+"' class='modal fade' role='dialog'><div class='modal-dialog'><div class='modal-content'><div class='modal-header'><button type='button' class='close' data-dismiss='modal'>&times;</button><h4 class='modal-title'>Anda yakin ingin menghapus?</h4></div><div class='modal-body'><p>File yang sudah dihapus tidak dapat di kembalikan.</p></div><div class='modal-footer'><form class='item-flex-group' action='/admin/mbojo-hapus/"+row.id+"' method='post'><input type='hidden' name='_token' value='"+csrf_token+"'><input type='hidden' name='_method' value='DELETE'><button class='btn btn-default' type='submit'>Delete <svg class='small-icon glyph stroked trash'><use xlink:href='#stroked-trash'></use></svg></button><button type='button' class='btn btn-default' data-dismiss='modal'>Close <svg class='small-icon glyph stroked cancel'><use xlink:href='#stroked-cancel'/></svg></button></form></div></div></div></div></div>";
}
